...
def follow_up(connection, cursor, sids):
    print("\nsid|descr|max_bid/rprice|day_left|hour_left|min_left|")
    for sid in sids:
        selection = '''
            SELECT s.sid, s.descr, ifnull(max(b.amount), 0) as max_bid, ifnull(s.rprice, 0) as res,
            ((strftime('%s',edate)-strftime('%s','now'))/86400) as day_left, 
            (((strftime('%s',edate)-strftime('%s','now'))%86400)/3600) as hour_left,
            ((((strftime('%s',edate)-strftime('%s','now'))%86400)%3600))/60 as min_left
            FROM sales s left outer join bids b using (sid)
            WHERE s.sid = ?
            GROUP BY s.sid, s.descr,day_left, hour_left, min_left;
            '''
        cursor.execute(selection,(sid,))
        out = cursor.fetchone()
...
def search_users(connection, cursor):
    ...
    while 1:
        ...
        if option == 'a' or option == 'A':
            ...
            for sale in result2:
                sids.append(sale[0])
            # if the user have no active listings
            if len(sids) == 0:
                print("The user have no active listings\n")
                return
            follow_up(connection, cursor, sids)
            break
...
def main():
    ...
    while 1:
        print("For the following possible operations, choose which one you want to perform")
